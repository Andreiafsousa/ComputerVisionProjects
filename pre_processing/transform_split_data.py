...
class LoadSplitTrainTest():
    ...
    def __call__(self, DATA_DIR: str, TEST_SPLIT: float):
        ...
        dataset = datasets.ImageFolder(DATA_DIR, transform=data_transforms, target_transform=lambda x: x)
        test_size = int(len(dataset) * TEST_SPLIT)
        train_dataset, test_dataset = torch.utils.data.random_split(dataset, [len(dataset) - test_size, test_size], generator=torch.Generator().manual_seed(42))

        val_size = int(len(train_dataset) * TEST_SPLIT)
        train_dataset, val_dataset = torch.utils.data.random_split(train_dataset, [len(train_dataset) - val_size, val_size], generator=torch.Generator().manual_seed(42))
